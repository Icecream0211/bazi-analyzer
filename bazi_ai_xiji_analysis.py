...
def bazi_xiji_ai_analysis_stream(result):
    ...
    现在请根据用户的八字信息进行喜忌分析，内容需包括以下几点：
    喜用神和忌用神：指出用户八字中最重要的喜用神和忌用神，并解释这些神在八字中的作用。
    流年影响：分析未来三年内流年对用户喜用神的影响，以及用户可能会经历的喜忌变化。
    幸运颜色和方位：根据用户八字和喜用神，提供适合用户的幸运颜色、方位，并说明如何利用这些喜忌元素提升运势。
    """

    systemPrompt = """
    你是一位精通八字命理的分析师，现在请根据用户的八字信息进行喜忌分析，不少于300字。
    """
    print(prompt)
    print()
    print(systemPrompt)
